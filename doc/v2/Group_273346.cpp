...
public:
	...
	/// </summary>
	/// <param name="infix_exp">�nix expression</param>
	...
	bool basic_validate_infix(vector<string> O)
	{
		//expr.erase(remove(expr.begin(), expr.end(), ' '), expr.end());
		string t1, t2;
	...
	ArExp infix_to_postfix()
	{
		...
		while ((tok = next_token(expr, i)) != "")
		{
			...
			else if (isOperator(tok))
			{
				...
				while (!S.empty() && isOperator(S.top()) && ((leftAssoc(tok[0]) && priority(tok[0]) <= priority(S.top()[0])) || (!leftAssoc(tok[0]) && priority(tok[0]) < priority(S.top()[0]))))
				{
					// ^ begin from right
					string s_top = S.top();
					O.push_back(s_top);
		...
		if (basic_validate_infix(O) == false)
		{
			throw InvalidExpressionException();
		}
